...
from lcasr.utils.utils import set_remote_paths, make_dir, load_yaml, load_pretrained_model
from lcasr.wrappers import create_msmd_train_loaders, get_model
...
def train_model(args, wandb_sweep=False, sweep_run_name=None):
    ...
    if not wandb_sweep:
        ...
        dump_path = os.path.join(model_tag_path, f'params{exp_tag}.pt')
        ...
        exp_tag = f"{'_finetune' if args.finetune else ''}"
        dump_path = os.path.join(run_dir, f'params{exp_tag}.pt')

    # getting dataloaders
    tr_loader, va_loader, tr_eval_loader = create_msmd_train_loaders(args)

    # get model
    model, loss = get_model(args)

    # load pretrained models if finetuning
    model = load_pretrained_model(args, model) if args.finetune else model
