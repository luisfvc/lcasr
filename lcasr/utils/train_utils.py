...
def iterate_dataset(model, dataloader, loss_function, optimizer=None, device=torch.device('cpu'),
                    retrieval_direction='a2s', has_rnn=True):
    """
    ...
    for data in tqdm(dataloader, ncols=70, total=len(dataloader), leave=False):

        scores_batch = [s.to(device, non_blocking=True) for s in data.scores] if has_rnn else data[0].to(device)
        specs_batch = [s.to(device, non_blocking=True) for s in data.specs] if has_rnn else data[1].to(device)

        with torch.set_grad_enabled(is_train):

            batch_x_embs, batch_y_embs = model(scores_batch, specs_batch)
        ...
        if is_train:
            ...
            if has_rnn:
                clip_grad_norm_(model.x_net.seq_model.parameters(), 0.5)
                clip_grad_norm_(model.y_net.seq_model.parameters(), 0.5)
    ...
    if retrieval_direction == 'a2s':
        metrics = compute_metrics(y_embs.cpu().numpy(), x_embs.cpu().numpy(), expand_metrics=False)
