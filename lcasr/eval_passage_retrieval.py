...
from lcasr.wrappers import create_test_loader, get_model
...
def eval_snippet_retrieval(args):
    ...
    model_tag = 'msmd_est_UV' if args.refine_cca else 'msmd'
    ...
    exp_tag = f"_embdim_{args.emb_dim}"
    exp_tag = f"{exp_tag}{'_finetune' if args.finetune else ''}"
    if args.finetune:
        exp_tag = f"{exp_tag}{'_fz' if args.freeze_cnn else ''}"
        exp_tag = f"{exp_tag}{'_cca' if args.refine_cca else ''}"

    model_path = os.path.join(model_path, f'params{exp_tag}.pt')

    # getting dataloaders
    te_loader, n_test, durations = create_test_loader(args)
    print(f'Number of candidates from the test set: {n_test}')
    ...
    for metrics, rd in zip(te_metrics, ret_dirs):
        ...
        # data = {'Cosine distance': cos_dists, '1/rank': aps, 'Passage duration': durations}
        # sns.scatterplot(data, x='Passage duration', y='Cosine distance', size='1/rank')
        # plt.show()
        ...
        for key, hits in hit_rates.items():
            recall_at_k[key] = float(100 * hits) / n_test
            pk = recall_at_k[key] / key
            print(f'\tTop {key:02d}: {recall_at_k[key]:.3f} ({hits}) {pk:.3f}')

        print('\n')
        print(f'\tMedian Rank: {med_rank_te:.2f} ({n_test})')
        print(f'\tMean Rank  : {mean_rank_te:.2f} ({n_test})')
        ...
        if args.dump_results:
            ...
            rd = rd.upper()
            dump_file = model_path.replace('params', 'eval').replace('.pt', f'_{args.dataset}_{rd}.yaml')
            # cos_dists_file = model_path.replace('params', 'cos_dists').replace('.pt', f'_{args.dataset}_{n_test}.npz')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--emb_dim', help='embedding dimension', type=int, default=64)
    ...
    parser.add_argument('--freeze_cnn', help='load pretrained encoder', action='store_true', default=False)
    parser.add_argument('--finetune', help='evaluate audio-finetuned model', action='store_true', default=False)
