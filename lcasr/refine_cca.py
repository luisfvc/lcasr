...
from lcasr.models.cca_layer import CCARefine
from lcasr.wrappers import create_snip_train_loaders, get_model
from lcasr.utils.utils import load_yaml, make_dir, set_remote_paths
...
def refine_cca(args, verbose=True, wandb_sweep=False, sweep_run_name=None):
    ...
    # getting dataloaders
    tr_loader = create_snip_train_loaders(args, only_refine=True)

    # get model
    model, _ = get_model(args, snippet_model=True)
    ...
    if not args.no_dump_model:
        if not wandb_sweep:
            ...
            refined_dump_path = os.path.join(model_path_est_UV, f'params.pt')
        else:
            refine_dump_name = f'params_est_UV.pt'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--finetune', help='load pretrained audio encoder', action='store_true', default=False)
    parser.add_argument('--ft_path', help='location of the finetuned audio model', type=str, default=None)
