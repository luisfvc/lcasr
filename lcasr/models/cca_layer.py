...
class CCALayer(nn.Module):
    ...
    def forward(self, H1=None, H2=None):
        ...
        if False:
            ...
            H1bar = H1 - self.mean1 if H1 is not None else None
            H2bar = H2 - self.mean2 if H2 is not None else None

            # transpose to formulas in paper
            H1bar = H1bar.t() if H1 is not None else None
            H2bar = H2bar.t() if H2 is not None else None

            U, V = self.U, self.V

        # re-project data
        lv1_cca = H1bar.t().matmul(U) if H1 is not None else None
        lv2_cca = H2bar.t().matmul(V) if H2 is not None else None
